...
parser.add_argument("--reranking", type=str, default="data/ranking_indices/dragon_roberta/dot_score/top_50_indices.json", help="Ranking result")
...
print(len(reranking_data))


# get sorted Top-30 indices list
top_k = 30
save_top_k_indices = []
for idx, file in enumerate(tqdm(files)):
    ...
    logits_list = []
    for rule_idx in reranking_data[idx][:top_k]:
    ...
    top_k_indices = torch.topk(logits_list, k=top_k).indices.tolist()
    final_top_k_indices = [reranking_data[idx][:top_k][i] for i in top_k_indices]
    save_top_k_indices.append(final_top_k_indices)
